...
LINUX = ENV.fetch('LINUX_HDR','/usr/include/linux')
...
JWT_SRC = File.join(JWT_DIR, 'libjwt')
...
ALL_INCLUDES = "-I #{INCLUDES} -I #{INCLUDE_DIR} -I #{CJSON_SRC} -I #{MQTT_HDR} -I #{JWT_DIR}/include"
...
File.open(TEST_MAKEFILE, "w") do |mkfile|
  ...
  mkfile.puts "TEST_CFLAGS ?= -std=c99 -DTEST -DCJSON"
  ...
  #mkfile.puts "\t${CC} -o $@ -c $< ${TEST_CFLAGS} -I #{JWT_DIR}/include -I #{CJSON_DIR}"
  #mkfile.puts ""
  ...
  test_sources.each do |test|
    ...
    mkfile.puts "\t${CC} -o $@ -c $< ${TEST_CFLAGS} -I #{SRC_DIR} -I #{MOCKS_DIR} -I #{UNITY_SRC} -I #{CMOCK_SRC} #{ALL_INCLUDES}"
    mkfile.puts ""
    ...
    mkfile.puts "\t${CC} -o $@ -c $< ${TEST_CFLAGS} -I #{SRC_DIR} -I #{UNITY_SRC} -I #{CMOCK_SRC} -I #{MOCKS_DIR} #{ALL_INCLUDES}"
    mkfile.puts ""

    # Build test suite executable
    test_objs = "#{test_obj} #{runner_obj} #{module_obj} #{mock_objs.join(' ')} #{linkonly_objs.join(' ')} #{UNITY_OBJ} #{CMOCK_OBJ} #{CJSON_OBJ}" #" #{JWT_OBJ}"
  ...
  all_headers_to_mock.each do |hdr|
    ...
    mkfile.puts "\t${CC} -o $@ -c $< ${TEST_CFLAGS} -I #{MOCKS_DIR} -I #{SRC_DIR} -I #{UNITY_SRC} -I #{CMOCK_SRC} #{ALL_INCLUDES}"
    mkfile.puts ""
  end
