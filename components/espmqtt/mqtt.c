...
static bool mqtt_connect(mqtt_client *client)
{
    ...
    client->mqtt_state.pending_msg_id = mqtt_get_id(client->mqtt_state.outbound_message->data,
                                        client->mqtt_state.outbound_message->length);
    mqtt_info("Password %s",client->connect_info.password);
    ...
    switch (connect_rsp_code) {
        ...
        case CONNECTION_REFUSE_NOT_AUTHORIZED:
            ...
            return false;
        default:
            mqtt_warn("Connection refused, Unknow reason %d",connect_rsp_code);
            return false;
    }
    return false;
}
...
{
	mqtt_info("HERE %s",settings->password);
    ...
    printf("settings password %s",settings->password);
...
void mqtt_publish(mqtt_client* client, const char *topic, const unsigned char *data, int len, int qos, int retain)
{
